...
module internal HttpClientGenerator =
    ...
    let extractHttpInformation (mem : Ident) (attrs : SynAttribute list) : HttpMethod * SynExpr =
        let matchingAttrs =
            attrs
            |> List.choose (fun attr ->
                match attr.TypeName.LongIdent |> List.last |> _.idText with
                | "Get"
                | "GetAttribute" -> Some (HttpMethod.Get, attr.ArgExpr)
                | "Post"
                | "PostAttribute" -> Some (HttpMethod.Post, attr.ArgExpr)
                | "Put"
                | "PutAttribute" -> Some (HttpMethod.Put, attr.ArgExpr)
                | "Delete"
                | "DeleteAttribute" -> Some (HttpMethod.Delete, attr.ArgExpr)
                | "Head"
                | "HeadAttribute" -> Some (HttpMethod.Head, attr.ArgExpr)
                | "Options"
                | "OptionsAttribute" -> Some (HttpMethod.Options, attr.ArgExpr)
                | "Patch"
                | "PatchAttribute" -> Some (HttpMethod.Patch, attr.ArgExpr)
                | "Trace"
                | "TraceAttribute" -> Some (HttpMethod.Trace, attr.ArgExpr)
                | _ -> None
            )
        ...
        | [] ->
            let allAttrs =
                attrs
                |> List.map (fun attr -> attr.TypeName.LongIdent |> List.map _.idText |> String.concat ".")
                |> String.concat "\n"

            failwith
                $"Required exactly one recognised RestEase attribute on member %s{mem.idText}, but got none.\n%s{allAttrs}"
        | matchingAttrs ->
            failwith
                $"Required exactly one recognised RestEase attribute on member %s{mem.idText}, but got %i{matchingAttrs.Length}"
    ...
    let createModule
        ...
        let nonPropertyMembers =
            ...
            |> List.map (fun mem ->
                let httpMethod, url = extractHttpInformation mem.Identifier mem.Attributes
        ...
        let createFunc =
            if spec.ExtensionMethods then
                ...
                let componentInfo =
                    SynComponentInfo.createLong interfaceType.Name
